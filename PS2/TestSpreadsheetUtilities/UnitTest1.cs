...
namespace PS2GradingTests
{
    ...
    public class DependencyGraphTest
    {
        ...
        /// <summary>
        /// Tests to be sure private data is kept private.
        /// </summary>
        [TestMethod]
        ...
        /// <summary>
        /// Tests each method in conjunction, in a readable manner.
        /// </summary>
        [TestMethod]
        public void ComprehensiveTest()
        {
            ...
            // We will give "a" many dependents, keeping track of those
            // and make "1" the dependee of many others
            HashSet<string> dents = new HashSet<string> { "b", "c", "d", "e", "f" };
            HashSet<string> dees = new HashSet<string> { "2", "3", "4", "5" };

            foreach (string s in dents)
            {
                dg.AddDependency("a", s);
            }
            foreach (string s in dees)
            {
                dg.AddDependency(s, "1");
            }

            Assert.AreEqual(dents.Count + dees.Count, dg.Size);
            ...
            Assert.IsTrue(dents.SetEquals(dg.GetDependents("a")));
            Assert.IsTrue(dees.SetEquals(dg.GetDependees("1")));

            // Test removing
            dg.RemoveDependency("foo", "bar");
            ...
            // Test replacing
            dents = new HashSet<string> { "b2", "c2", "d2"};
            dees = new HashSet<string> { "j", "k", "l"};
            dg.ReplaceDependents("a",dents);
            dg.ReplaceDependees("1", dees);
